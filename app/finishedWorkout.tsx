...
const App = () => {
  ...
  const workout = wId ? getOneWorkout(wId.toString()) : undefined;
  const exercises = wId ? getExercisesFromWorkout(parseInt(wId.toString())) : [];

  const totalDuration = exercises.reduce((sum, ex) => sum + ex.duration, 0);
  const totalTime = workout ? totalDuration + (exercises.length - 1) * workout.rest : totalDuration;
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <ThemedText style={styles.title} type="title">Workout Complete!</ThemedText>
        <View style={[styles.summaryCard, { backgroundColor: cardTheme.background, borderColor: cardTheme.border }]}>
          ...
          <View style={styles.summaryRow}>
            ...
            <ThemedText>{totalTime} S</ThemedText>
          </View>
        </View>
